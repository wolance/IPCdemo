...
DWORD WINAPI Send(LPVOID lpParameter )
{
	while(1)
	{
		if(WaitNamedPipe(NAMEDPIPEB,NMPWAIT_WAIT_FOREVER))
		{
	...
	HANDLE hPipe = CreateFile(NAMEDPIPEB,GENERIC_READ|GENERIC_WRITE,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
...
{
	HANDLE hPipe = CreateNamedPipe(NAMEDPIPEA,PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,0,1,1024,1024,0,0);
	if(INVALID_HANDLE_VALUE == hPipe)
	{
